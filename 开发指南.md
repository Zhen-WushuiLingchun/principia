...
-   **核心组件**:
    ...
    -   `HandwritingCanvas.tsx`: 自定义 HTML5 Canvas 实现，支持多页手写、压感模拟和橡皮擦逻辑。
...
-   **后端**: `/api/convert` 端点使用专门的 Prompt 指示 LLM 将 LaTeX 中的 `\textcolor{color}{text}` 映射为 Markdown 中的 `<font color="color">text</font>`（或保留兼容的 LaTeX 语法），反之亦然。
